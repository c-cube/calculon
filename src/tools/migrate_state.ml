...
let () =
  ...
  let opts = [ "-db", Arg.Set_string db, " DB file" ] |> Arg.align in
  ...
  let tr_plugin (n, sub) =
    ...
    if n = "factoids" then (
      ...
      let stmt =
        DB.prepare db {| INSERT INTO factoids(key,value) VALUES (?,?); |}
      in
      List.iter
        (fun (key, value) ->
      ...
      |}
      |> check_db_;
      DB.exec db {| CREATE INDEX IF NOT EXISTS idx_social on social(name); |}
      |> check_db_;
      ...
      let stmt =
        DB.prepare db "INSERT INTO plugins(name,value) VALUES (?,?);"
      in

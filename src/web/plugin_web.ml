...
let page_title ~with_description uri : string option =
  ...
  | Some title, Some description ->
    Some (Format.asprintf "%s : %s" title description)
  | Some title, None -> Some (Format.asprintf "%s" title)
  | _, _ -> parse body $ "title" |> leaf_text
...
let cmd_yt_search =
  Command.make_simple_l ~prio:10 ~cmd:"yt_search" ~descr:"lookup on youtube"
    (fun _ s ->
      ...
      let urls = find_yt_ids ~n:1 body in
      List.fold_left
        (fun acc url ->
          Log.debug (fun k -> k "Getting metadata for url: `%s`" url);
          match page_title ~with_description:false (Uri.of_string url) with
          | Some x ->
  ...
  let search s : string option =
    ...
    try
      ...
      | l ->
        ...
        Log.info (fun k ->
            k "giphy: pick `%s` in list of len %d" r.Giphy_j.url (List.length l));
    ...
    Command.make_simple ~prio:10 ~cmd:"giphy"
      ~descr:"lookup on giphy (Powered by Giphy)" (fun _ s ->
        ...
        if s = "" then
          ...
          match search (String.trim s) with
          | Some x -> Some x
          | None -> Some "not found"
        ))
end
...
let find_emoji (s : string) : string option =
  ...
  try
    ...
    | Some (em, title), _ -> Some (Printf.sprintf "%s: %s (%s)" em title query)

...
type t = {
  ...
  db_file: string;  (** Database path. @since 0.8 *)
  _hidden: hidden;
      (** This field is present to prevent the user from using a literal record
          to build configuration. This way, adding new fields doesn't break
          existing code.
...
val of_argv : ?extra_args:(string * Arg.spec * string) list -> unit -> t

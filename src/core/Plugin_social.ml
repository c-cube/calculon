  ...
  try
    {
      ...
      to_tell =
        ...
        |> J.convert_each (fun j ->
               ...
               | [ from; on_channel; msg ] ->
                 { from; on_channel; msg; tell_after = None }
               ...
               | _ ->
                 raise
                   (Bad_json
                      (spf "bad `tell` object: %s" (Yojson.Safe.to_string j))));
      ignore_user =
        (match J.member "ignore_user" json with
        | `Null -> false
        | v -> J.to_bool_option v |> Option.value ~default:false);
    }
    |> fun x -> Ok x
  with
  | Bad_json s -> Error s
  ...
  `Assoc
    [
      ...
      ( "to_tell",
        `List
          (List.map
             (fun { from; on_channel; msg; tell_after } ->
               let last =
                 match tell_after with
                 | None -> []
...
let prepare_db (self : t) : unit =
  DB.exec self
    ...
    |}
  |> check_db_ self;
  DB.exec self {| CREATE INDEX IF NOT EXISTS idx_social on social(name); |}
  |> check_db_ self;
  ()
...
let is_contact (self : t) nick : bool =
  ...
  let@ stmt =
    with_stmt self {| SELECT EXISTS (SELECT * from social WHERE name=?) |}
  in
...
let new_contact (self : t) nick : contact =
  let nick = String.lowercase_ascii nick in
  let d = { last_seen = now (); to_tell = []; ignore_user = false } in
...
let data (self : t) nick : contact option =
  ...
  if is_contact self nick then (
    let@ stmt = with_stmt self {| SELECT value FROM social WHERE name=? |} in
    DB.bind_text stmt 1 nick |> check_db_ self;
    DB.step stmt |> check_db_ self;
    match
    ...
    with
    | _, Ok c -> Some c
    | s, Error err -> failwith (spf "invalid contact %S: %s" s err)
    | exception _ -> failwith (spf "cannot access contact %S" nick)
  ) else
    None
...
let update_data' self nick ~f : unit =
  ignore (update_data self nick ~f : contact)
...
let cmd_tell_inner ~at (self : t) =
  Command.make_simple
    ~descr:
      ("ask the bot to transmit a message to someone absent\n"
      ^
      if at then
        "format: <date> <nick> <msg>"
      else
        "format: <nick> <msg>")
    ~prio:10
    ~cmd:
      ...
      try
        let dest, msg, tell_after =
          if at then (
            ...
            let t =
              match
                Ptime.of_rfc3339 ~strict:false t |> Ptime.rfc3339_error_to_msg
              with
              | Ok (t, _, _) -> Ptime.to_float_s t
              | Error (`Msg msg) -> failwith (spf "invalid timestamp: %s" msg)
            in
            d, m, Some t
          ) else (
            let d, m =
        ...
        in
        update_data' self dest ~f:(fun state ->
            {
              state with
              to_tell =
...
let cmd_seen (self : t) =
  Command.make_simple_l
    ~descr:"ask for the last time someone talked on this chan" ~prio:10
    ~cmd:"seen" (fun _msg s ->
      try
        let now = now () in
  ...
  Command.make_simple_l
    ...
    ~prio:10 ~cmd:"last" (fun _msg s ->
      try
        ...
        let top_n =
          try
            match int_of_string dest with
            | x when x > 0 -> x
            | _ -> default_n
          with Failure _ -> default_n
        in
...
let cmd_ignore_template ~cmd prefix_stem ignore (self : t) =
  Command.make_simple ~descr:(cmd ^ " nick") ~prio:10 ~cmd (fun _ s ->
      try
        let dest = String.trim s in
...
let cmd_ignore_list (self : t) =
  Command.make_simple_l ~descr:"add nick to list of ignored people" ~prio:10
    ~cmd:"ignore_list" (fun _ _ ->
      try
...
let on_message (self : t) (module C : Core.S) msg : _ Lwt.t =
  ...
  let nick, channel =
    ...
    | Msg.JOIN (_, _) | Msg.PRIVMSG (_, _) ->
      let msg =
        Core.privmsg_of_msg msg |> Option.get_or "message parsing error"
      in
      ( Some msg.nick,
        if Core.is_chan msg.to_ then
          Some msg.to_
        else
          None )
    | Msg.NICK newnick -> Some newnick, None
    | _ -> None, None
  in
  ...
  | Some nick ->
    ...
    let now = now () in
    let contact =
      update_data self nick ~f:(fun st -> { st with last_seen = now })
    in
    let to_tell, remaining =
      contact.to_tell
      |> List.partition (fun t ->
             match t.tell_after, channel with
             | _, None -> false
             | None, _ -> true
             | Some f, Some chan ->
               (* delay expired, and it was on the same channel *)
               CCFloat.(now > f) && String.equal t.on_channel chan)
    in
...
let plugin =
  ...
  in
  Plugin.db_backed ~commands ~prepare_db
